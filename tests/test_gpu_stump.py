...
def test_gpu_stump_self_join(T_A, T_B, cooperative):
    m = 3
    ...
    comp_mp = gpu_stump(pd.Series(T_B), m, ignore_trivial=True, cooperative=cooperative)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp, comp_mp)
...
@pytest.mark.parametrize("cooperative", (True, False))
def test_gpu_stump_self_join_larger_window(T_A, T_B, m, cooperative):
    if len(T_B) > m:
        zone = int(np.ceil(m / 4))
        ref_mp = naive.stamp(T_B, m, exclusion_zone=zone)
        comp_mp = gpu_stump(T_B, m, ignore_trivial=True, cooperative=cooperative)
        naive.replace_inf(ref_mp)
        naive.replace_inf(comp_mp)
...
@pytest.mark.parametrize("T_A, T_B", test_data)
@pytest.mark.parametrize("cooperative", (True, False))
def test_gpu_stump_A_B_join(T_A, T_B, cooperative):
    m = 3
    ref_mp = naive.stamp(T_B, m, T_B=T_A)
    comp_mp = gpu_stump(T_B, m, T_A, ignore_trivial=False, cooperative=cooperative)
    naive.replace_inf(ref_mp)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp, comp_mp)
...
@pytest.mark.parametrize("T_A, T_B", test_data)
@pytest.mark.parametrize("cooperative", (True, False))
def test_parallel_gpu_stump_self_join(T_A, T_B, cooperative):
...
@pytest.mark.parametrize("T_A, T_B", test_data)
@pytest.mark.parametrize("cooperative", (True, False))
def test_parallel_gpu_stump_A_B_join(T_A, T_B, cooperative):
    device_ids = [device.id for device in cuda.list_devices()]
    if len(T_B) > 10:
        m = 3
        ref_mp = naive.stamp(T_B, m, T_B=T_A)
...
@pytest.mark.parametrize("cooperative", (True, False))
def test_gpu_stump_constant_subsequence_self_join(cooperative):
    T_A = np.concatenate((np.zeros(20, dtype=np.float64), np.ones(5, dtype=np.float64)))
    m = 3
    zone = int(np.ceil(m / 4))
    ref_mp = naive.stamp(T_A, m, exclusion_zone=zone)
    comp_mp = gpu_stump(T_A, m, ignore_trivial=True, cooperative=cooperative)
    naive.replace_inf(ref_mp)
    naive.replace_inf(comp_mp)
...
@pytest.mark.parametrize("cooperative", (True, False))
def test_gpu_stump_one_constant_subsequence_A_B_join(cooperative):
    ...
    comp_mp = gpu_stump(T_A, m, T_B, ignore_trivial=False, cooperative=cooperative)
    naive.replace_inf(ref_mp)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp[:, 0], comp_mp[:, 0])  # ignore indices
...
@pytest.mark.parametrize("cooperative", (True, False))
def test_gpu_stump_two_constant_subsequences_A_B_join(cooperative):
...
@pytest.mark.parametrize("cooperative", (True, False))
def test_gpu_stump_identical_subsequence_self_join(cooperative):
...
@pytest.mark.parametrize("cooperative", (True, False))
def test_gpu_stump_identical_subsequence_A_B_join(cooperative):
    identical = np.random.rand(8)
    T_A = np.random.rand(20)
...
@pytest.mark.parametrize("cooperative", (True, False))
def test_gpu_stump_nan_inf_self_join(
    ...
    for substitution_location_B in substitution_locations:
        ...
        comp_mp = gpu_stump(T_B_sub, m, ignore_trivial=True, cooperative=cooperative)
        naive.replace_inf(ref_mp)
        naive.replace_inf(comp_mp)
        npt.assert_almost_equal(ref_mp, comp_mp)
...
@pytest.mark.parametrize("cooperative", (True, False))
def test_gpu_stump_nan_zero_mean_self_join(cooperative):
    T = np.array([-1, 0, 1, np.inf, 1, 0, -1])
    m = 3

    zone = int(np.ceil(m / 4))
    ref_mp = naive.stamp(T, m, exclusion_zone=zone)
    comp_mp = gpu_stump(T, m, ignore_trivial=True, cooperative=cooperative)
