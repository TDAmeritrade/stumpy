...
import pandas as pd
from stumpy import stump_uint, config
import pytest
import naive
...
def test_stump_self_join(T_A, T_B):
    ...
    comp_mp = stump_uint(T_B, m, ignore_trivial=True)
    naive.replace_inf(ref_mp)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp, comp_mp)

    comp_mp = stump_uint(pd.Series(T_B), m, ignore_trivial=True)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp, comp_mp)
...
def test_stump_A_B_join(T_A, T_B):
    ...
    comp_mp = stump_uint(T_A, m, T_B, ignore_trivial=False)
    naive.replace_inf(ref_mp)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp, comp_mp)

    comp_mp = stump_uint(pd.Series(T_A), m, pd.Series(T_B), ignore_trivial=False)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp, comp_mp)
...
def test_stump_constant_subsequence_self_join():
    ...
    comp_mp = stump_uint(T_A, m, ignore_trivial=True)
    naive.replace_inf(ref_mp)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp[:, 0], comp_mp[:, 0])  # ignore indices

    comp_mp = stump_uint(pd.Series(T_A), m, ignore_trivial=True)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp[:, 0], comp_mp[:, 0])  # ignore indices
...
def test_stump_one_constant_subsequence_A_B_join():
    ...
    comp_mp = stump_uint(T_B, m, T_A, ignore_trivial=False)
    naive.replace_inf(ref_mp)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp[:, 0], comp_mp[:, 0])  # ignore indices
...
def test_stump_two_constant_subsequences_A_B_join():
    ...
    comp_mp = stump_uint(pd.Series(T_B), m, pd.Series(T_A), ignore_trivial=False)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp[:, 0], comp_mp[:, 0])  # ignore indices
...
def test_stump_nan_inf_self_join(T_A, T_B, substitute_B, substitution_locations):
    ...
    for substitution_location_B in substitution_locations:
        ...
        comp_mp = stump_uint(T_B_sub, m, ignore_trivial=True)
        naive.replace_inf(ref_mp)
        naive.replace_inf(comp_mp)
        npt.assert_almost_equal(ref_mp, comp_mp)

        comp_mp = stump_uint(pd.Series(T_B_sub), m, ignore_trivial=True)
        naive.replace_inf(comp_mp)
        npt.assert_almost_equal(ref_mp, comp_mp)
...
def test_stump_nan_inf_A_B_join(
    ...
    for substitution_location_B in substitution_locations:
        for substitution_location_A in substitution_locations:
            ...
            comp_mp = stump_uint(T_A_sub, m, T_B_sub, ignore_trivial=False)
            naive.replace_inf(ref_mp)
            naive.replace_inf(comp_mp)
            npt.assert_almost_equal(ref_mp, comp_mp)
...
def test_stump_nan_zero_mean_self_join():
    ...
    comp_mp = stump_uint(T, m, ignore_trivial=True)
