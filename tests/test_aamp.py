...
@pytest.mark.parametrize("T_A, T_B", test_data)
def test_aamp_A_B_join_p_norm_2(T_A, T_B):
    m = 3
    ref_mp = naive.aamp(T_A, m, T_B=T_B, p_norm=2.0)
    comp_mp = aamp(T_A, m, T_B, ignore_trivial=False, p_norm=2.0)
    naive.replace_inf(ref_mp)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp, comp_mp)
...
@pytest.mark.parametrize("T_A, T_B", test_data)
def test_aamp_A_B_join_p_norm_3(T_A, T_B):
    m = 3
    ref_mp = naive.aamp(T_A, m, T_B=T_B, p_norm=3.0)
    comp_mp = aamp(T_A, m, T_B, ignore_trivial=False, p_norm=3.0)
    naive.replace_inf(ref_mp)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp, comp_mp)
    comp_mp = aamp(pd.Series(T_A), m, pd.Series(T_B), ignore_trivial=False, p_norm=3.0)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp, comp_mp)


@pytest.mark.parametrize("T_A, T_B", test_data)
def test_aamp_A_B_join_p_norm_1(T_A, T_B):
    m = 3
    ref_mp = naive.aamp(T_A, m, T_B=T_B, p_norm=1.0)
    comp_mp = aamp(T_A, m, T_B, ignore_trivial=False, p_norm=1.0)
    naive.replace_inf(ref_mp)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp, comp_mp)
    comp_mp = aamp(pd.Series(T_A), m, pd.Series(T_B), ignore_trivial=False, p_norm=1.0)
    naive.replace_inf(comp_mp)
    npt.assert_almost_equal(ref_mp, comp_mp)
